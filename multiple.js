...
const createSession = function(id, description) {
  ...
  client.on('message', async msg => {
    ...
    // console.log('MESSAGE RECEIVED', msg);
     const pesan = msg.body.toLowerCase()
       ...
       if (pesan == 'ping'){
        client.sendMessage(msg.from, pesan)
     ...
     else if (pesan.startsWith('!join ')) {
         const inviteCode = pesan.split(' ')[1];
         try {
         ...
         const buttons_reply_call_url = new Buttons(await MessageMedia.fromUrl('https://via.placeholder.com/350x150.png') , 
         [{body: "Test 2 Call", url: "https://google.com"}],  
          
         'xs') // Reply button with call button & url button
 
          client.sendMessage(msg.from, buttons_reply_call_url);
     } 
     else if (pesan === '!reaction') {
             msg.react('👍');
     }
 });
  client.initialize();
